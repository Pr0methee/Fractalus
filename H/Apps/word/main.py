...
#from H.Apps.word.executors import PythonExecutor, SQLExecutor, ask_bd,ask_csv,ShowCSV,LatexExecutor
...
FILES = [
    ("All",'.txt .py .pyw .csv .html .css .js .php .sql .tex .sty'),
    ("Text files",'.txt'),#
    ("CSV",".csv"),#
    ("Python files",'.py .pyw'),#
    ("HTML files",".html"),#
...
class App(Text_edit.TextEdit):
    def __init__(self,master,link:str='',ask=[
        ...
        for desc in [("Ouvrir",self.askopen,'Ctrl+o'),("Enregistrer",self.save,'Ctrl+s'),("Enregistrer Sous",self.saveas,''),("Nouveau",self.new_file,'Ctrl+n')]:
